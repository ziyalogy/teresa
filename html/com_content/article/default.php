...
if ($imagesBg=='') {
	$imagesBg = JURI::base(true) .'/images/joomlart/others/bg-masthead.jpg';
}

// put heading to masthead
$overrideMasthead = true;
$modules = JModuleHelper::getModules('masthead');
if ($modules && $tmpl !== 'component') {
	$menu = $app->getMenu()->getActive();

	if ($menu && $menu->link === 'index.php?option=com_content&view=article&id=' . $this->item->id) {
		$module = array_shift($modules);
		$moduleParams = new JRegistry($module->params);
		$item_id = (array) $moduleParams->get('menuitem_config')->item_id;

		foreach ($item_id as $key => $ids) {
			if (in_array($menu->id, $ids)) {
				$overrideMasthead = false;
			}
		}
	}

	if ($overrideMasthead) {
		$masthead = JLayoutHelper::render('joomla.content.masthead', array('params' => $this->params, 'title' => $this->escape($this->item->title), 'print' => $this->print, 'item' => $this->item,'params' => $params));
...
?>
<div class="com-content-article detail-page item-page<?php echo $this->pageclass_sfx; ?>" itemscope itemtype="https://schema.org/Article">
	...
	<?php if (!$overrideMasthead || $tmpl == 'component') : ?>
